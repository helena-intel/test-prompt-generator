...
ADDITIONAL_PREFIXES = {
    "en": ["TEXT: ", "The text to process:", "The text: "],
    "zh": ["这是文本：", "正文", "这是需要处理的文本：", "本句之后是要处理的文本。"],
}


def generate_prompt_from_text(tokenizer, num_tokens, source_text, prefix, additional_prefixes, no_length_check=False):
    new_prefix = prefix
    new_source_text = new_prefix.strip() + " " + source_text if prefix is not None else source_text
    inputs = tokenizer(new_source_text)
    ...
    ### Postprocessing
    # if prompt.endswith(" ") or prompt.endswith(",") and "t5" in tokenizer.name_or_path.lower():
    ...
    if "chatglm" in tokenizer.name_or_path.lower():
    ...
    if num_prompt_tokens != num_tokens:
        ...
        lang = "zh" if _chinese_char_re.search(source_text) else "en"
        try:
            additional_prefix = additional_prefixes[lang].pop(0)
            prefix = additional_prefix if prefix is None or prefix in ADDITIONAL_PREFIXES[lang] else f"{prefix} {additional_prefix}"
            return generate_prompt_from_text(tokenizer, num_tokens, source_text, prefix, additional_prefixes, no_length_check)

        except IndexError:
            if not no_length_check:
                message = f"Expected {num_tokens} tokens, got {num_prompt_tokens}. Tokenizer: {tokenizer.name_or_path}. "
                message += "Set `no_length_check` or `--no-length-check` to disable this check."
...
def generate_prompt(
    ...
    tokenizer.model_max_length = 25000

    prompt_dicts = []
    for i, tok in enumerate(num_tokens):
        # additional_prefixes = {f"{prefix} {key}: value for key, value in ADDITIONAL_PREFIXES.items()}
        prompt, num_prompt_tokens = generate_prompt_from_text(
            tokenizer=tokenizer,
            num_tokens=tok,
            source_text=source_text,
            prefix=prefix,
            additional_prefixes=copy.deepcopy(ADDITIONAL_PREFIXES),
            no_length_check=no_length_check,
        )
